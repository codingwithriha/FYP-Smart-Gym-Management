...
def login(root, role, username_input, password_input):
    ...
    try:
        ...
        # ---------------- MANAGER LOGIN (FIXED 🔥) ----------------
            ...
            cursor.execute(
                "SELECT id FROM users WHERE username=%s AND password=%s AND role='Manager'",
            ...
            if user:
                ...
                # Step 2: Get manager_id using user_id
                cursor.execute(
                    "SELECT manager_id FROM managers WHERE user_id = %s",
                    (user_id,)
                )
        ...
        if user:
            ...
            if role == "admin":
                ...
                manager_dashboard.open_manager_dashboard(user["manager_id"])

            elif role == "member":
                import member.member_dashboard as member_dashboard
                member_dashboard.open_member_dashboard(user["member_id"])

            elif role == "trainer":
                import trainer.trainer_dashboard as trainer_dashboard
                trainer_dashboard.open_trainer_dashboard(user["trainer_id"])

            elif role == "attendant":
                import attendant.attendant_dashboard as attendant_dashboard
                attendant_dashboard.open_attendant_dashboard(user["attendant_id"])
...
def open_login_page():
    ...
    tk.Label(root, text="Select Role", bg="#252540", fg="white",
    ...
    tk.Label(root, text="New User?", bg="#252540", fg="white",
