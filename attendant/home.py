...
def load_home(main_frame, attendant_id):
    ...
    for i in range(4):
        ...
        tk.Label(top, text=card_icons[i], bg=CARD_BG, fg=PRIMARY, font=("Segoe UI", 20)).pack(side="left")
        tk.Label(top, text=card_titles[i], bg=CARD_BG, fg=MUTED, font=("Segoe UI", 11)).pack(side="left", padx=8)
    ...
    for label, value in [("Name", name), ("Email", email), ("Shift", shift)]:
        ...
        tk.Label(row, text=f"{label}:", bg=LEFT_BG, fg=MUTED, font=("Segoe UI", 10)).pack(side="left")
        tk.Label(row, text=value, bg=LEFT_BG, fg=TEXT, font=("Segoe UI", 11, "bold")).pack(side="left", padx=5)
    ...
    for act in [
        "Mark Attendance", "Report Equipment Issue"
    ]:
        tk.Button(
            actions, text=act,
            bg=PRIMARY, fg="white",
    ...
    def load_equipment_status():
        ...
        cursor.execute("""
            ...
            FROM equipment WHERE zone_id=%s
        """, (zone_id,))
        for r in cursor.fetchall():
            tree.insert("", "end", values=(r["ID"], r["Name"], r["Status"]))
        conn.close()

    tk.Button(btns, text="Zone Attendance", bg=PRIMARY, fg="white",
              font=("Segoe UI", 11, "bold"), relief="flat",
              command=load_zone_attendance).pack(side="left", padx=5)

    tk.Button(btns, text="Equipment Status", bg=PRIMARY, fg="white",
              font=("Segoe UI", 11, "bold"), relief="flat",
              command=load_equipment_status).pack(side="left", padx=5)
